...
# This is how I build my distros from scratch
...
echo Building $projectUpper $ver
cd /usr/local/share/
mkdir $project
cd $project
echo "version=$ver" > $project.conf
chown www-data:www-data project.conf

# Detect hardware
if [ ! -z $2 ]; then
  echo $2 > /etc/.$project_hw_model_identifier
fi
...
echo "$projectUpper $ver compiled."
echo ""
